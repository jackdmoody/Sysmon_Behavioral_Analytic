...
def check_h2_role_separation(df, labels):
    ...
    # Check that role and global baselines actually differ
    # (if all hosts are in one role, role baseline == global baseline)
    minority_role = counts.idxmin()
    majority_role = counts.idxmax()
    minority_pct  = counts.min() / counts.sum()
    if minority_pct < 0.05:
        print(f"{FAIL}  Role '{minority_role}' has only {minority_pct:.1%} of hosts — "
              "too small to build a meaningful peer baseline")
        return False

    print(f"{PASS}  Smallest role has {minority_pct:.1%} of hosts (>5% threshold)")
    return True
...
def check_h4_channel_variance(df, labels):
    ...
    # Also check fused != any single channel (Borda is actually combining)
    if art.seq_scores is not None and "S_seq" in art.seq_scores.columns and triage is not None:
        ...
        corr   = merged["score"].corr(merged["S_seq"])
        if corr > 0.999:
